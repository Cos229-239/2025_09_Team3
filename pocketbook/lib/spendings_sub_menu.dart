...
class SpendingsSubmenu extends StatefulWidget {
  const SpendingsSubmenu({
    ...
    this.focusedMonth, // New optional parameter for month filter
  });
...
class _SpendingsSubMenuState extends State<SpendingsSubmenu> {
  ...
  Future<void> _loadTxns() async {
    ...
    try {
      ...
      final rows = widget.categoryName == 'All Categories'
          ? await handler.getSpendingLog(DatabaseHandler.userID) // Fetch all logs for month view
          : await handler.getSpendingInCategory(DatabaseHandler.userID, widget.categoryName);
      final catRows = await handler.getCategoriesFromName(DatabaseHandler.userID, widget.categoryName);
                ...
                Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      Text(
                        'Spent: ${_fmt(_monthSpent)}',
                        style: TextStyle(color: onC, fontSize: 16),
                      ),
                    ],
                  ),
                ),
